...
# Use pre-trained VGG-16 CNN
vgg = keras.applications.vgg16.VGG16(weights='imagenet')

# Define a function that will predict the probability of a cat on the image. Since ImageNet contains a number of classes for cats, indexed from 281 to 294, add probabilities for those classes to get overall 'cat' probability:
def predict(img):
...
# Build a heatmap of probabilities, dividing the image into n x n squares:
...
# In this example, use dense neural network. Use stochastic gradient descent optimizer and mean squared error (MSE) as the metrics because the task is regression.
...
# Train the network. Normalize the input data (by subtracting mean and dividing by standard deviation) for slightly better performance.
imgs_norm = (imgs-np.mean(imgs))/np.std(imgs)
model.fit(imgs_norm,bb,epochs=30)

# See how it translates into more tangible metrics such as mAP. First define IOU metric between two bounding boxes:
def IOU(bbox1, bbox2):
...
# Generate 500 test images, and plot first 5 of them to visualize how accurate we are. We will print out IOU metric as well.
...
# Calculate mean precision over all cases, we just need to go over all our test samples, compute IoU, and calculate mean:
