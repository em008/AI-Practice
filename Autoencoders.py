...
# Denoising
# Autoencoders can be effectively used to remove noise from images. 
def noisify(data):
...
# Super-resolution
# We can train autoencoders to increase the resolution of the image
...
# VAE is the autoencoder that learns to predict statistical distribution of the latent parameters (latent distribution)
...
# Reconstruction loss is the loss function that shows how close reconstructed image is to the target (can be MSE)
# KL loss, which ensures that latent variable distributions stays close to normal distribution
@tf.function
def vae_loss(x1,x2):
